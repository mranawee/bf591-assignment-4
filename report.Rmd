...
*Hint*: make sure your `plot_sample_distributions()` reshapes your count matrix from a _wide_ format, to a _long_ format. 
```{r}
...
*Hint*: pay close attention to the `prcomp` documentation for running PCA, and ensure you're providing the count matrix in the correct orientation for running PCA over the _samples_ in `plot_pca()`.
```{r}
...
Now that we've explored our dataset prior to count normalization, we should normalize our data to see how things change. A simple and intuitive approach to count normalization is _counts per million_: that is finding the number of counts for a given gene in a given sample for each million reads observed. It can be easily calculated using this formula
